...
class StudyGroupRepository {
    ...
    var groupInfo = listOf<PostStruct>()
    var authorInfo = listOf<PostAuthorStruct>()

    /** @param groupId 해당하는 그룹의 게시 글을 가져옴
     * @param mostRecentPostTimeStamp 새로 고침 용 타임 스탬프. 해당 시간 이후의 게시글을 새로 가져옴. 기본값은 "" : 모든 시간대의 글을 가져옴*/
    suspend fun fetchData(groupId: String, mostRecentPostTimeStamp: String = ""){
        Log.d("LOG_CHECK", "StudyGroupRepository :: fetchData() -> fetch group($groupId) data")

        val value = postDB
            .whereEqualTo(FIELD.GROUP.ID, groupId)
            .whereGreaterThan(FIELD.POST.CREATE_AT, mostRecentPostTimeStamp)
        ...
        withContext(Dispatchers.IO) {
            ...
            val tempAuthorList = MutableList(originList.size) { PostAuthorStruct() }
            ...
            if(tempGroupList.size > 0){
                ...
                authorInfo = tempAuthorList + authorInfo
                groupInfo = tempGroupList + groupInfo
            }
        }
    }
    ...
    suspend fun viewMyPost(authorId: String): Pair<List<PostStruct>, List<PostAuthorStruct>>?{
        val authorList: List<PostAuthorStruct>?
        val postList = groupInfo.filter { post ->
    ...
    suspend fun searchPostByTitle(searchTitle: String): Pair<List<PostStruct>, List<PostAuthorStruct>>?{
        val postList = groupInfo.filter { post->
        ...
        val authorList = mutableListOf<PostAuthorStruct>()
    ...
    private suspend fun getAuthor(authorId: String) =
        userDB.document(authorId)
            .get()
            .await()
            .toObject(PostAuthorStruct::class.java)?: PostAuthorStruct()
